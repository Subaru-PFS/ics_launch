...
__conda_setup="$('/software/stack_2026-02-10/conda/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/software/stack_2026-02-10/conda/etc/profile.d/conda.sh" ]; then
        . "/software/stack_2026-02-10/conda/etc/profile.d/conda.sh"
    else
        export PATH="/software/stack_2026-02-10/conda/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<

conda activate lsst-scipipe-12.1.0
